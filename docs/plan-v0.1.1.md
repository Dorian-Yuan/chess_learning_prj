...
**目标定位**：沉浸式、交互式国际象棋自主学习与对弈 Web 平台  
...
### 1.1 项目背景与核心痛点
国际象棋（Chess）作为全球普及度极高的智力博弈运动，拥有严谨的规则体系与深厚的战术底蕴。初学者在入门与进阶初期常面临以下痛点：
1. **规则体系繁杂且特殊规则抽象**：王车易位、兵的升变、吃过路兵、逼和（Stalemate）与长将判和等规则机制独特，若无可视化动态演示与即时互动操作，极易产生规则盲区与误解。
2. **传统教学偏重说教缺乏互动**：常见入门教材多为静态图文或被动视频讲解，学习者无法在接触规则的第一时间在棋盘上即时走子验证，认知反馈链路较长。
3. **缺乏渐进式人机梯级训练**：主流商业对战平台门槛高、计算依赖云端服务器或需要联网注册，初学者难以在低压力、即开即用、可回溯步法的私有环境中进行练习。
4. **多端访问繁琐**：个人本地开发环境需要通过 Tailscale 等私有组网工具在手机、平板或笔记本上随时随地无缝访问。

### 1.2 用户画像（User Persona）
* **姓名**：李明（化名）
* **棋类背景**：棋类爱好者 / 国际象棋零基础至初级进阶玩家，渴望系统化掌握国际象棋的规则、战术与开局。
* **学习诉求**：从零系统学习国际象棋，通过直观的步法交互练习加深对规则与战术的理解，并能随时与不同段位 AI 进行复盘对战。
...
graph TD
    ...
    Root --> M3[模块 3: 开局原理与经典开局]
    Root --> M4[模块 4: 核心战术与残局练习]
    Root --> M5[模块 5: 多难度 AI 对战与复盘]

    M1 --> M1_1[兵/马/象/车/后/王 单子走法与吃子练习]
    M2 --> M2_1[王车易位 / 兵升变 / 吃过路兵 / 将死与逼和]
    M3 --> M3_1[开局三原则 / 意大利开局 / 西西里防御 / 后翼弃兵等]
    M4 --> M4_1[击双 / 牵制 / 抽将 / 闪击 / 引入 / 消除保护]
    M5 --> M5_1[Stockfish WASM 4档难度 / 实时走法提示 / PGN导入导出]
```
...
5. **和棋（Draw）与逼和（Stalemate）**：
   * **逼和（Stalemate）**：轮到走棋一方**未被将军**，但全盘**无任何合法着法**，判为和棋。
...
* **首页 / 仪表盘 (`/`)**：学习路径总览、快捷进入对战与每日一题、基础规则与战术模块导览。
...
```
chess_learning_prj/
├── CHANGELOG.md                           # 项目更新日志与版本记录
├── docs/                                  # 项目规划与技术设计文档
│   ├── plan-v0.1.0.md                     # v0.1.0 初始基线规划文档（保留历史版本）
│   └── plan-v0.1.1.md                     # v0.1.1 架构修订规划文档（当前版本）
├── public/                                # 静态资源目录（不经打包工具转译）
│   ├── favicon.svg                        # 网站图标（SVG 矢量方案）
...
│   └── assets/
│       ├── logo/                          # 项目 Logo 与品牌资产
│       │   ├── logo.png                   # 项目主 Logo（Editorial 复古风格）
│       │   └── favicon.png                # 备用位图图标
...
### 5.1 选定理由
1. **契合棋类文化沉淀**：国际象棋拥有上千年的历史底蕴，`Editorial` 风格独特的衬线字体、经典墨黑、羊皮纸暖白与胡桃木色调，能够传递出沉稳、专注、典雅的书卷气与博弈氛围。
...
### 5.6 Logo 与品牌标识规范（Brand Identity）
为保障项目在视觉传达上的严谨性与典雅感，项目设立专属的 Logo 与品牌资产规范：
1. **生成与管理机制**：
   * 项目 Logo 由 AI 生图工具（agy `generate_image`）定制生成，原始高清资产保存于 `public/assets/logo/logo.png`。
   * 同时衍生并配置 1:1 正方形图标 `favicon.png`，网站核心 Favicon 采用矢量 `public/favicon.svg` 方案以确保在任意分辨率屏幕及高分屏下极致锐利清晰。
2. **设计规范与视觉特征**：
   * **风格定位**：严格遵循 StyleKit `Editorial` 编辑部复古美学，融合经典木版雕刻/学院纹章（Academic Crest）质感。
   * **配色沿用**：背景使用羊皮纸暖白（`#F9F8F6` 系），主体采用牛津墨蓝（`#14213D`）与典雅复古金（`#C29B38`）双色协同，与全站 Design Tokens 深度契合。
   * **核心意象**：以国际象棋标志性棋子（如马 Knight、王 King 或王冠 Crown）结合展开的书卷（Book）或学院盾徽（Shield Crest）意象，象征“智力博弈、系统研习、典雅从容”。
   * **排版与构图**：正方形（1:1）对称稳健构图，去除冗余杂色，细节在 128px 小尺寸及导航栏中依然清晰可辨；无文字或仅保留极简字母；严禁任何 Emoji 或扁平卡通低幼风格。
3. **应用场景**：
   * **顶部导航栏（Navbar）**：尺寸 `32px * 32px`，搭配衬线体项目名称，建立专业品牌认知。
   * **首页欢迎区（Hero Section）**：大尺寸展示，强化典雅博弈与学习氛围。
   * **浏览器 Favicon / PWA 图标**：适配现代浏览器标签页与移动端快捷方式。
...
[步骤 0: 规划文档 (本阶段)]
       │
       ▼
[步骤 1: 骨架工程与基础架构] ──► 搭建 Vite+TS 项目、集成 CSS Tokens、引入静态与品牌Logo资源
       │
       ▼
...
[步骤 3: 棋子与特殊规则互动教学] ──► 编写 6 大棋子与 4 大特殊规则关卡与判定器
       │
       ▼
[步骤 4: 战术训练与开局库] ──► 实现 50+ 战术解谜互动与经典开局分支浏览器
       │
       ▼
[步骤 5: Stockfish 人机对战] ──► 接入 WASM Worker、评估条、多难度与 PGN 工具
       │
       ▼
[步骤 6: 跨端适配、Tailscale 联调与交付] ──► 响应式优化、音效合成、内网远程全功能验收
```
...
| :--- | :--- | :--- | :--- | :--- |
| **Step 0** | **前期规划** | 需求梳理、技术选型、UI 规范定制、编写规划书 | `docs/plan-v0.1.1.md` | 文档涵盖所有 7 大章节，逻辑闭环，架构严谨 |
| **Step 1** | **骨架搭建** | 初始化 Vite + TS 工程，配置 tokens/reset 样式，集成 SVG 棋子、品牌 Logo 与 Lucide 图标 | 基础运行工程、SPA 路由框架、响应式布局外壳、Logo 资产 | 页面可正常启动，Header/导航可切换，Logo 与样式无错乱 |
...
| **Step 4** | **开局与战术模块** | 开发战术解谜界面（击双/牵制等）与经典开局谱系浏览器 | `/tactics` 视图、`/openings` 视图、50+ 题库数据 | 解谜支持单步验证、最佳防守反击与提示；开局支持逐步演绎 |
| **Step 5** | **Stockfish 对战** | 引入 Stockfish 18 WASM Worker，实现 4 档难度、局势评估条、悔棋、复盘 | `/play` 视图、`stockfish_worker.ts`、评估条组件 | AI 能在 1 秒内响应走子，难度梯度明显，评估条准确更新 |
| **Step 6** | **调优与远程验证** | 移动端触控优化、PWA 离线支持、Tailscale 远程跨设备实机测试与联调 | 最终交付 Web 应用、优化报告 | 在手机/iPad 上通过 Tailscale IP 完美流畅运行 |
...
- [ ] **品牌与 Logo 规范**：集成 Editorial 风格的专属品牌 Logo 与 Favicon，各尺寸与高分屏下清晰无变形。
